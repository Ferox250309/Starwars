...
waiting_background = """
    <style>
    .stApp {
        background-image: url("https://github.com/Vroomfrondal/Star-Wars-Weather/blob/main/images/loading-bg.png?raw=true");
...
with col7:
    city = st.text_input("Enter city", key='city_input', help='')
    st.markdown(
        """
        <style>
        div[role="textbox"] > div:first-child {
            width: 600px;
        }
        </style>
        """,
...
if not city:
    st.markdown(waiting_background, unsafe_allow_html=True)
    with col4:
        html_string = f"<div style='display: flex; flex-direction: column; justify-content: center; align-items: center; height: 85vh;'><h1 style='text-align: center; color: white; font-family: Arial; letter-spacing: 0.43em; font-size: 115px; font-weight: 100; text-transform: uppercase;'>Waiting...</h1></div>"
        st.markdown(html_string, unsafe_allow_html=True)
else:
    apilink = str("https://api.weatherapi.com/v1/current.json?key=36fec4787ae4493db02202139232005&q=") + str(city) + str("&aqi=no")
    html_string = f"<div style='display: flex; flex-direction: column; justify-content: center; align-items: center; height: 85vh;'><h1 style='text-align: center; color: white; font-family: Arial; letter-spacing: 0.43em; font-size: 115px; font-weight: 100; text-transform: uppercase;'>Waiting...</h1></div>"
    st.markdown(html_string, unsafe_allow_html=True)
    st.markdown(
        """
